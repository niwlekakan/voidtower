...
5. **Verify** (`verification-loop`): `scripts/devteam/gates.sh` must pass locally — fmt, clippy `-D warnings`, tests, plus any task-specific gates. Fix until green; if blocked 3 attempts, escalate. If a *pre-existing* file outside your task's scope needs a fmt/clippy fix to get `gates.sh` green, that means you ran a whole-crate command (see the Hard Rules formatting bullet) — undo it, `git checkout -- <that file>`, and fix only what your own diff introduced.
6. **Self-check before committing**: run `git diff --stat main...HEAD` yourself and read the file list. Every file must be one you meant to touch for this task. If anything unexpected shows up (a file outside "Files to touch", an unrelated reformat), that's a bug in what you just did, not a fact to commit — fix it before proceeding, don't rationalize it into the commit message.
7. **Commit** with Conventional Commits. Update docs touched by the change. Push the branch (`git push -u origin devteam/<task-id>-<slug>`) — if this is a retry and the branch name already has commits on `origin` from an earlier abandoned attempt, `git push --force-with-lease` is the correct, expected resolution, not a blocker to escalate. **Do not open the PR yourself, by any mechanism** — not `gh pr create`, not a raw `curl`/GitHub-API call using `$GITHUB_TOKEN` as a workaround when `gh` isn't installed. `automerge` or the operator opens and merges PRs from outside the sandbox; this boundary exists so review always happens on a diff nobody already has a stake in landing. Put your spec-compliance notes and any scope judgment calls in the commit body.
8. **Stop.** Do not start another task. Do not review your own PR.
...
- **Formatting is diff-scoped.** `gates.sh` checks only files your diff touches, and its own G0 step already runs `rustfmt` on exactly those files — you never need to invoke a formatter yourself. **Never run `cargo fmt` or `cargo fmt --all`** (it reformats the whole crate via the `mod` graph and will touch forbidden zones and unrelated files alike). This has been the single most common real-world mistake across P0 tasks: a worker runs `cargo fmt` to silence one clippy lint, and ships an unrelated whole-file reformat (in one recorded case, ~1173 lines in a single file) that gets correctly rejected by review. If `gates.sh` reports pre-existing debt in a file you didn't mean to touch, that means a whole-crate command already ran — undo it, don't compound it.
...
**Phase exits are yours alone** (G4): VoidTower runs on any hardware, so this is a software review, not a real-homelab checklist — read the remaining full-review PRs for the phase, confirm the phase's exit criteria in `docs/gap-analysis.md` are met (tests/gates green, no open escalations), then `touch .devteam/phase-exit/<PHASE>.ok`, from outside the forge. The loop can empty its queue; it cannot declare a phase done. (P3's device-agent work is the one exception — its own exit criteria in §5 genuinely need a second physical/VM machine, since that's what the feature is.)
