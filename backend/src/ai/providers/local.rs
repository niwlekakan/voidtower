...
impl AiProvider for LocalLlmProvider {
    ...
    async fn complete(&self, req: &AiRequest) -> std::result::Result<String, String> {
        ...
        let client =
            egress::client_for_local(&self.base_url, std::time::Duration::from_secs(120)).await?;
    ...
    async fn stream(&self, req: &AiRequest) -> std::result::Result<reqwest::Response, String> {
        ...
        let client =
            egress::client_for_local(&self.base_url, std::time::Duration::from_secs(300)).await?;
        ...
        if !resp.status().is_success() {
            return Err(format!(
                "Local LLM request failed with HTTP {}",
                resp.status()
            ));
        }
        Ok(resp)
    ...
    async fn health_check(&self) -> std::result::Result<(), String> {
        let client =
            egress::client_for_local(&self.base_url, std::time::Duration::from_secs(3)).await?;
