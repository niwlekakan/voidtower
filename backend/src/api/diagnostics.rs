    ...
    if !p.exists() {
        ...
        if writable {
            ...
            DiagCheck::warn(
                ...
                "Config directory",
                "Config",
                format!("{} exists but is not writable", p.display()),
                None,
            )
        }
    }
...
fn check_apps_dir(cfg: &crate::config::Config) -> DiagCheck {
    ...
    if p.exists() {
        ...
        DiagCheck::warn(
            ...
            "Config",
            format!(
                "{} does not exist — App Vault deployments may fail",
                p.display()
            ),
...
fn check_frontend(cfg: &crate::config::Config) -> DiagCheck {
    ...
    if !p.exists() {
        ...
        DiagCheck::warn(
            ...
            "Frontend assets",
            "Config",
            format!("{} exists but index.html is missing", p.display()),
            None,
        )
    } else {
        DiagCheck::pass(
...
fn check_systemd() -> DiagCheck {
    if crate::services::is_systemd_available() {
        DiagCheck::pass(
            ...
            "systemd",
            "Services",
            "systemd is available".to_string(),
        )
    } else {
        DiagCheck::warn(
            "systemd",
...
fn check_port(cfg: &crate::config::Config) -> DiagCheck {
    ...
    match TcpListener::bind(format!("{}:{}", cfg.bind, cfg.port)) {
        Ok(_) => DiagCheck::warn(
            ...
            "Network",
            format!(
                "Port {} is free — VoidTower is not listening in this process context",
                cfg.port
            ),
            None,
        ),
        Err(_) => DiagCheck::pass(
            "port",
            "Bind port",
            "Network",
            format!(
                "Port {} is in use — VoidTower (or another process) is listening",
                cfg.port
            ),
        ),
    }
