...
pub async fn image_generate(
    ...
    if backend == "comfyui"
        || (backend == "auto"
            && probe("http://localhost:8188/system_stats").await.is_some()
            && probe("http://localhost:7860/sdapi/v1/options")
                .await
                .is_none())
    {
...
async fn image_generate_comfyui(
    ...
    let seed = req
        .seed
        .unwrap_or_else(|| rand::random::<i32>() as i64 & 0x7FFFFFFF);
    ...
    let queued: serde_json::Value = resp
        .json()
        .await
        .map_err(|e| AppError::Internal(e.into()))?;
    ...
    loop {
        ...
        if hdata.get(&prompt_id).is_none() {
            continue;
        }
...
async fn save_image(state: &AppState, b64: &str) -> Result<Json<ImageGenResponse>> {
    ...
    let raw = if let Some(pos) = b64.find(',') {
        &b64[pos + 1..]
    } else {
        b64
    };
