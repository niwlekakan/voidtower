...
pub async fn update_check(
    ...
    if is_dev_install() {
        ...
        let branch = git(&root, &["rev-parse", "--abbrev-ref", "HEAD"])
            .unwrap_or_else(|_| GITHUB_BRANCH.into());
        ...
        let ahead = git(
            &root,
            &["rev-list", &format!("{remote_ref}..HEAD"), "--count"],
        )
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(0);
        ...
        let commit_hash = std::fs::read_to_string(dir.join(".commit"))
            ...
            .trim()
            .to_string();
        let current = if commit_hash.len() >= 7 {
            commit_hash[..7].to_string()
        } else {
            commit_hash
        };
        let latest = github_latest_commit();
        let can_update = latest
            .as_deref()
        ...
        let error = if latest.is_none() {
            Some("Could not reach GitHub API".to_string())
        } else {
            None
        };
        Ok(Json(UpdateCheck {
...
kill -TERM {pid}\n"
    ...
    std::process::Command::new("setsid")
        ...
        .map_err(|e| AppError::Internal(e.into()))?;
    Ok(Json(
        serde_json::json!({ "ok": true, "message": "Updating… VoidTower will restart when done." }),
    ))
}
