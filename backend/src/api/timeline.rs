...
fn classify(action: &str, resource_type: Option<&str>) -> &'static str {
    if action.starts_with("login")
        || action.starts_with("logout")
        || action.starts_with("bootstrap")
        || action.starts_with("create_user")
        || action.starts_with("delete_user")
        || action.starts_with("change_password")
        || action.starts_with("revoke_session")
    {
        return "auth";
    }
...
pub async fn list(
    ...
    qb.push(" ORDER BY a.timestamp DESC LIMIT ")
        .push_bind(limit)
        .push(" OFFSET ")
        .push_bind(q.offset);
    ...
    let mut count_qb: QueryBuilder<'_, Sqlite> =
        QueryBuilder::new("SELECT COUNT(*) FROM audit_log a");
