...
Physical-disk attributes use the CMDB reconciliation vocabulary: `SIZE` is emitted as `attributes.capacity_bytes`, `TRAN` as `attributes.protocol`, `ROTA` as `attributes.rotation`, and bounded `SERIAL`/`WWN` values are also retained in `attributes` alongside their identity evidence. `SIZE` must be a positive JSON integer; missing, null, zero, negative, and string values fail closed. The collector does not emit a `size_bytes` alias. This keeps the collector output aligned with physical-disk classification and automatic/trusted-provider registration. `runtime` contains only collection-runtime facts; it is not used for physical-type classification.
