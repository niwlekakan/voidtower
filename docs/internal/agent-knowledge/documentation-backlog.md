...
- Retrospective: independent review found and the implementation fixed unsafe source/catalog tar extraction, matrix checksum manifest collisions, false-positive systemd detection, version-drifting fallback from a requested release to `main`, v-prefixed release filename mismatch, unsupported armv7 advertisement, incomplete documented unit guard, offline network fallback, checksum case/duplicate handling, offline MCP pre-cache, reset agent ordering, and ignored catalog extraction failures. The reusable archive seam is `scripts/install.sh::validate_tar_archive`; the release workflow now computes one basename-compatible manifest from the downloaded archives and runs the package contract gate before builds.
