...
- No V6-01 product-contract implementation started because the approved handoff still requires protected activation of `.github/workflows/compatibility-enforcement.yml` before the remaining schema/generated-client work.
- Fresh external evidence at `2026-09-21T15:21:58Z`: `origin/dev` remains `b9a24729c2a7750900f285d61daa4439e0cd95f9`; the verifier workflow is absent from the remote `dev` tree; the public workflow-runs endpoint returns HTTP 404; the public branch-protection endpoint returns HTTP 401; and `gh` is unavailable. No protected run ID or required-status context(s) are observable in this sandbox.
- Local evidence passed: repository truth (`source_inventory_only`), compatibility inventory (`status: passed`, `unknown: []`), repository hygiene (`670 tracked files checked`), schema migration ownership, and `git diff --check` before continuity edits. These checks do not establish protected activation.
- Maturity remains `blocked`; runtime/provider/browser/Docker/install/upgrade/recovery/release qualification was not attempted. The next dependency is operator-side protected-branch publication and observation, followed by V6-01 source-owned schemas, generated clients/OpenAPI, negotiation/errors, SSE recovery, drift tests, and documentation.

## V6-01 durable-operation envelope sub-slice — 2026-09-21

- Backend source-owned v1 envelopes now cover job list/read/success and approval list/read/success response seams. Canonical handlers and `backend/src/api/operation_adoption.rs` emit the same job success shape with `schema_version`, `resource_id`, `action`, and `job`.
- Approval decisions validate a 500-Unicode-character comment bound and distinguish missing records (`404 not_found`), no-longer-pending decisions (`409 approval_conflict`), and redacted internal failures instead of collapsing all failures into conflict.
...
- Source-owned API v1 now covers resource list/read/capability responses and retained event history. Backend handlers emit typed envelopes with `schema_version`, bounded `limit` values (`1..=500`), non-negative event cursors, and event retention bounds; invalid query values return `400 bad_request` before reads.
- Frontend `api.resources.list/get/capabilities` and `api.events.history` use generated-contract-backed parsers and typed responses. Parsers reject malformed resource identity, alias/capability ownership, actor types, event sequence ordering, and cursor bounds; client methods reject invalid pagination before fetch.
- Real-router focused tests cover exact resource/event envelopes, alias/resource identity, invalid limits/cursors, and session-protected reads. Frontend tests cover parser rejection, encoded URLs, and client-side pagination validation.
- Evidence after this continuation: focused backend event/version tests, focused frontend contract/client/event tests, frontend type-check/lint/build, manifest-scoped Rust formatting, generated-contract drift, repository truth, and diff checks passed. Runtime/browser/release qualification remains unclaimed; protected verifier activation remains unresolved from the prior handoff.

## V6-01 action and inventory contract continuation — 2026-09-21

- The authenticated node upload now returns a typed `InventoryUploadEnvelopeV1` with `schema_version: 1` and a nested `InventorySnapshotResultV1`; `backend/src/api/cmdb/inventory.rs` no longer serializes an untyped `Value` response. The agent transport validates the response schema before checking the acknowledged snapshot identity.
- Inventory request structs (`InventorySnapshotV1`, host/entity observations, and identity evidence) reject unknown JSON fields with `deny_unknown_fields`; existing UUID, timestamp, text, count, JSON-depth, duplicate-key, node/path binding, replay/conflict, and administrator-field protections remain in force.
- `backend/contracts/api-v1-envelope-contract.json` is extended with the inventory success envelope and `scripts/generate-api-contract.mjs` regenerated `frontend/src/api/generatedApiContract.ts`. The frontend parser validates schema version, snapshot identity, replay flag, and non-negative bounded result counts.
- Focused RED/GREEN evidence: the unknown-field contract test first failed because serde discarded the field, then passed after fail-closed deserialization was added; version, inventory-router, agent-transport, frontend parser, and generated-contract tests passed. Runtime agent service, browser, provider, Docker, installation, upgrade/recovery, and release qualification remain unclaimed.

- Next dependency-ready slice: complete source-owned action/inventory schemas and generated/OpenAPI ownership, or obtain operator-side protected verifier activation evidence before widening V6-01. Do not start V6-02 web qualification.

## V6-01 action-plan schema and OpenAPI continuation — 2026-09-21

- `backend/src/operations/schemas.rs` is the shared durable-action boundary: every registered durable action must expose matching `<action>.input.v1` and `<action>.result.v1` identities, canonical object input is bounded to 64 KiB, eight nested levels, 64 properties/items per level, 128-character keys, and 4 KiB strings, and result persistence is validated before completion.
- `backend/src/operations/invocation.rs` rejects invalid action input before resource planning/provider adapter execution; the real Axum router returns `400 invalid_action_input` and creates neither a job nor an adapter call. `backend/src/operations/worker.rs` rejects over-bounded action results before result persistence.
- `backend/contracts/api-v1-envelope-contract.json` is the source-owned v1 contract for the complete `PlanViewV1`, canonical plan/submit request and response paths, bounded error/version envelopes, and OpenAPI 3.1 components. `scripts/generate-api-contract.mjs` deterministically generates both `frontend/src/api/generatedApiContract.ts` and `docs/api-v1.openapi.json`; `--check` fails on drift.
- Frontend `parsePlanSuccessEnvelope` now validates the complete plan shape, action-bound schema identities, nested operation/policy bounds, resource/action binding for plan and job envelopes, and `frontend/src/api/types.ts` exposes the corresponding typed plan response.
- Focused and final evidence passed after cleaning only disposable `/tmp/vt-p1-*` and `/tmp/voidtower-*` test fixtures: backend full gate (705 unit + 2 golden-path integration tests), strict Clippy, rustfmt, frontend full gate (13 files/83 tests, type-check, lint, build), generated-contract drift, repository truth, compatibility inventory, migration ownership, and diff checks. The first concurrent full-gate attempt hit the 512 MiB `/tmp` ceiling and is recorded as an environment failure, not product evidence.
- Maturity is `integration-verified` for this backend/frontend contract boundary. Runtime/provider/browser/Docker/installation/upgrade/recovery/release qualification and protected verifier activation remain unclaimed or blocked. Untracked `testing/` and Python cache paths remain preserved and are not part of this slice.

## V6-01 durable result and envelope hardening — 2026-09-22

- Worker completion and reconciliation now validate raw registered action results before recursive sanitization, then persist only the bounded canonical result. Fallible diagnostic/result sanitization propagates errors and prevents an unsafe value from reaching durable job, event, or audit writes.
- Frontend job parsing now rejects impossible progress (`current > total` or total unequal to plan step count), contradictory result/error payloads, terminal states missing their required result/error, and event records with unknown keys. Approval reads require the complete source-owned approval record; collection and success envelopes remain exact-key and version checked.
- Focused evidence after the hardening passed: `npx vitest run src/api/envelopeClient.test.ts src/api/operationsClient.test.ts --reporter=verbose` (60 tests), frontend type-check/lint/build, worker result/redaction tests, full backend test/clippy/format gates, generated-contract and repository checks. Runtime/browser/provider/Docker evidence remains unavailable in the sandbox.
- Exact configured secret values are redacted at the provider adapter boundary; the worker cannot independently compare arbitrary result fields against secret-manager values because no secret material is available at completion. The worker therefore adds credential-shaped key redaction and bounded diagnostics; this is a documented limitation, not a runtime qualification claim.

## V6-01 action-plan schema and approval hardening final — 2026-09-22

- Verified implementation commit `9e66e4769054811be864fd9e4e104179d57ec7c8` completes the bounded durable-action contract family across Rust invocation/worker/approval boundaries, source/generated API contracts, frontend parsers/tests, and `docs/api.md`.
- `backend/src/operations/schemas.rs` resolves schema IDs through the immutable action registry, rejects forged metadata, bounds canonical JSON input/results, and rejects null results. Adapter-owned action validation remains the field/value boundary before provider access.
- Worker and reconciliation result/diagnostic sanitization is fallible and fail-closed; persisted results/errors are bounded and redacted before atomic completion. Approval expiry and late decisions distinguish `expired` from `stale`; a process-wide async write mutex plus conditional updates serialize the single-process SQLite controller's decision/expiry writers.
- Final evidence: backend `cargo test --all-targets --all-features` passed 711 unit tests plus 2 golden-path integration tests and examples with `RUST_TEST_THREADS=2`; strict Clippy and rustfmt passed; frontend 13 files/87 tests, type-check, lint, and build passed; generated contract, repository truth, compatibility inventory, migration ownership, diff, and staged credential scans passed. Independent staged-diff review passed with no security or logic findings.
- Maturity is `integration-verified` for this backend/frontend contract boundary. Runtime, browser, provider, Docker, installation, upgrade/recovery, protected verifier activation, and release qualification remain unclaimed; `/tmp` ENOSPC was an environment-only failed attempt and was resolved by removing disposable test fixtures before rerunning.
- Next dependency-ready slice: operator-side protected verifier activation or host runtime/browser qualification for this completed contract boundary; do not widen into V6-02 web functionality until those gates are available.
