...
- Add one shared stream-credential helper in `api/events.rs` for session cookie, Authorization
  bearer, and query-token compatibility.
...
- Update `docs/api-tokens.md` with `alerts:read`, query-token compatibility, positive session roles,
  and emergency-disable behavior across durable aliases.
