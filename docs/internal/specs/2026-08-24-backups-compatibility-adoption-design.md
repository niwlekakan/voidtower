...
- Preserve legacy HTTP request bodies, but return the versioned durable job envelope
  `{ "schema_version": 1, "resource_id": "...", "action": "...", "job": ... }` from all five
  execution routes.
...
All five return only `202` with the versioned envelope
`{ "schema_version": 1, "resource_id": "...", "action": "...", "job": <canonical job summary> }`
after accepted submission. They do
