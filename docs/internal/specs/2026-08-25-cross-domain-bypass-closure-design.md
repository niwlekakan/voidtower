...
- Convert `POST /api/apps/:project_name/expose` to `proxy.rule.create`. It returns the canonical
  versioned `202` job envelope and no longer writes `proxy_configs`, nginx configuration, or audit success
  from the handler.
...
- versioned `202` job-envelope acceptance and every stable job state;
