...
- the existing optional `token` query parameter for browser `EventSource` clients that cannot set
...
Session cookies, `Authorization: Bearer`, and the existing `?token=` compatibility path are parsed
by one shared authentication helper. Durable event delivery and cursor handling are implemented
once and mounted at both durable routes; the integrations alias does not own a second history loop
